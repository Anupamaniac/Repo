package com.rosettastone.succor.controls
{
	...
	import com.rosettastone.succor.renderer.MultiSelectItemRenderer;
	import com.rosettastone.succor.skins.MultiSelectionDropDownListSkin;
		
	[Event(name="addItem", type="flash.events.Event")]						
	
	public class MultiSelectionDropDown extends DropDownList
	{
		private var dataGroupLocal:DataGroup;
		...
		protected function activateAllCheckBoxes():void {
			...
			for (var c:int = 0; c < dataGroup.numElements; c++) {
				...
				if (obj) {
					...
					obj.addEventListener(MouseEvent.MOUSE_DOWN, mouseRichText, false, 0, true);
					obj.addEventListener(MouseEvent.MOUSE_UP, mouseRichText, false, 0, true);
		 ...
		 */
		public function deselectAllCheckBoxes():void {
			currentlySelectedCheckBoxes = [];
			if(dataGroupLocal != null){
				for (var c:int = 0; c < dataGroupLocal.numElements; c++) {
					var obj:MultiSelectItemRenderer = dataGroupLocal.getElementAt(c) as MultiSelectItemRenderer;
					if (obj) {
						obj.checkbox.selected = false;
					}
				}
			}
			labelDisplay.text ="Select Actions";
		}
		...
		/**
		 * on click RichText of DropDown (dispatches event for toggling the status of checkbox)
		 * 
		 * @param change event
		 */
		protected function mouseRichText( event:Event ):void {
			event.currentTarget.checkbox.selected = !(event.currentTarget.checkbox.selected);
			event.currentTarget.checkbox.dispatchEvent(new Event(Event.CHANGE, true, false));
			dispatchEvent(new Event("selectionChange"));
		...
		protected function changeCheckBoxSelection(event:Event):void {
			...
			if ( multiSelect.length > 1) {
				str += "Multiple Actions Selected";
			}else if (multiSelect.length == 1 ) {
				var str1:String = multiSelect[0].label;
				str += str1;
			}else{
				str += "Select Actions";
			}
